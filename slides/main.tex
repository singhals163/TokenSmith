...
the pages are independent. Restructure as map-reduce, splitting the PDF,
running \texttt{docling} workers in parallel and sorting based on offset.
...
\textbf{Pros}
\begin{itemize}
  \item I/O-bound; saturates all CPU cores cheaply
  \item Zero ML dependencies (no PyTorch, no model load)
  \item Slightly higher answer quality on this corpus
...
\textbf{Cons}
\begin{itemize}
  \item Spatially blind: two-column layouts get stitched
        horizontally and silently corrupt the text
  \item Regex headings are brittle: ``\texttt{Chapter 1:}'',
        broken-line headers, extra whitespace all get missed
  \item No text layer means no output (scanned PDFs)
\end{itemize}
\end{column}
\end{columns}
...
\textbf{Partial solution.} Speed depends on a clean single-column
native-text PDF. Next step: run \texttt{docling} on the GPU and
measure layout-aware extraction at real speed.

\note{Be honest here. pypdfium2 is fast on this textbook because
the textbook is well-formatted; on a two-column paper or a
non-standard heading style, the regex silently corrupts the
output. The real fix is to make the layout-aware tool fast,
which is the docling-on-GPU experiment teed up next.}
\end{frame}
...
\begin{itemize}
  \item For clean single-column native-text PDFs, \texttt{pypdfium2}
    is roughly 1{,}100$\times$ faster than the original sequential
    \texttt{docling} and slightly better on answer quality. For
    visually complex documents (multi-column papers, scanned PDFs,
    non-standard heading styles) \texttt{docling} is still required;
    the next experiment is to make it fast on the GPU.
  \medskip
  ...
  \item TokenSmith now runs end to end on a 4-core / 8\,GiB consumer
    machine for textbooks that fit the fast path; layout-aware
    extraction is the remaining floor.
\end{itemize}
