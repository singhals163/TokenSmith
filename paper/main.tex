...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{figs/extraction_scaling.png}
...
\texttt{book\_authors}, where the retrieval pulls incorrect chunks and the generator then hallucinates an entirely
...
to within float rounding because their
inference is dominated by integer-quantized ops and is deterministic across GPU silicon. The two FP32 backends drift by about
0.025 points: \texttt{minilm} from 0.603 (V100) to 0.578 (RTX 6000),
\texttt{nomic\_st} from 0.589 to 0.563. 
The drift comes from small floating-point differences in the embedder
and the reranker on different GPU silicon.
