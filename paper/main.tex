...
can compare lighter alternatives across the same hardware. The indexer
...
metric (final 0.558). Its single worst question is
\texttt{book\_authors}, where the retrieval pulls chunks 145, 1396,
1260, and so on rather than the front-matter author chunks (ideal IDs
0, 1, 2, 3, 22), and the generator then hallucinates an entirely
unrelated Stephen Hawking book; the final score on that question is
0.071 with zero of seven reference keywords matched. The other
lightweight variants also struggle on \texttt{book\_authors} (MiniLM
0.42, Nomic-ST 0.26, with the 4\,B baseline topping out at 0.47), so
the question is hard for everyone, but GPT4All falls furthest.
...
\texttt{torch.cuda.set\_per\_process\_memory}\\\texttt{\_fraction(8/24)} for any
...
uses less than 3\,GiB. The Vulkan (kompute) path used by the GPT4All
backend has the smallest VRAM footprint of the four (2.1\,GiB) and
provides a CUDA-free option for non-NVIDIA silicon.

\paragraph{Why the final scores differ slightly from
Table~\ref{tab:phase3-v100}.}
The GGUF-based variants (\texttt{qwen4b} and
\texttt{nomic\_gpt4all\_gpu}) score the same on V100 and on RTX 6000
to within float rounding (0.592 / 0.558 in both tables) because their
inference is dominated by integer-quantized ops and is essentially
deterministic across GPU silicon. The two FP32 backends drift by about
2.5 points: \texttt{minilm} from 0.603 (V100) to 0.578 (RTX 6000),
\texttt{nomic\_st} from 0.589 to 0.563. Same model weights, same FAISS
indexes (the V100 indexes are reused), same generator, same scorer.
The drift comes from small floating-point differences in the embedder
(query side) and the reranker on different GPU silicon, which
propagate through the top-50 ordering, then the top-5, then the
generated answer, and finally the score. Both tables are valid
measurements; Table~\ref{tab:phase3-v100} reports the original V100
quality study and Table~\ref{tab:rtx-capped} reports the consumer-GPU
re-run under the 8\,GiB cap. Cap on or off, the consumer numbers are
identical (the cap only reserves VRAM and does not change numerics);
the comparison to V100 is the meaningful one.
