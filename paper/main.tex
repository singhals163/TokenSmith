...
\begin{tikzpicture}[
  ...
  \node[font=\small\bfseries, anchor=south west]
    at ($(pdf.north west)+(-0.1,0.4)$) {Cold ingestion (once per textbook)};
  ...
  % --- Per-query row ---
  \node[io]    (q)   at (0, -3.4)  {Question};
  \node[stage] (qe)  at (2.4, -3.4){Embed query};
  \node[stage] (ret) at (5.8, -3.4){Retrieve\\\footnotesize FAISS + BM25\\\footnotesize top 50};
  \node[stage] (rer) at (9.2, -3.4){Rerank\\\footnotesize cross-encoder\\\footnotesize top 5};
  \node[stage] (gen) at (12.6, -3.4){Generate\\\footnotesize Qwen2.5-1.5B};
  \node[io]    (a)   at (15.5, -3.4){Answer};
  ...
  \node[font=\small\bfseries, anchor=south west]
    at ($(q.north west)+(-0.1,0.4)$) {Per query (each question)};
  \node[font=\scriptsize\bfseries, color=blue!60!black,
...
used to embed the question. \shivam{``Cold ingestion (once per textbook)"
text in the very beginning is overlapping with the first few boxes of extract and chunk phases, fix it.}}
\label{fig:pipeline}
\end{figure*}
...
\shivam{Can you add relevant links to the relevant code here? Like the repo link (my personal fork), and various branches and what is present on those branches, only mention the ones that are relevant that we setup earlier in the day for final submission?}
